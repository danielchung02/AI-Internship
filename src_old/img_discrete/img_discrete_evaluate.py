"""Evaluate a frame-stack, discrete-action Double-DQN checkpoint."""
...
def main() -> None:
    ...
    env = make_image_env()
    try:
        agent = ImageDQNAgent(env.action_space.n, env.observation_space.shape, device)
    ...
    result = {
        ...
        "seed_start": args.seed_start, "action_repeat": action_repeat,
        "mean_reward": float(values.mean()),
        "std_reward": float(values.std()), "min_reward": float(values.min()),
        "max_reward": float(values.max()), "completed_laps": completed_laps,
