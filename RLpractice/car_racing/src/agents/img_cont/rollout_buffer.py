"""On-policy rollout storage for frame-stack image PPO."""
...
class RolloutBuffer:
    """Stores uint8 image stacks and computes GAE-Lambda advantages."""

    def __init__(self, capacity: int, image_shape: tuple[int, ...], action_dim: int) -> None:
        if capacity <= 0 or action_dim <= 0:
            raise ValueError("capacity and action_dim must be positive")
        self.capacity = capacity
        self._images = np.empty((capacity, *image_shape), dtype=np.uint8)
    ...
    def get(
        ...
        *,
        ...
        for index in range(self._size - 1, -1, -1):
            ...
            delta = self._rewards[index] + gamma * bootstrap * self._next_values[index] - self._values[index]
            gae = delta + gamma * gae_lambda * continuation * gae
            advantages[index] = gae

        returns = advantages + self._values[: self._size]
        batch = RolloutBatch(
            images=torch.as_tensor(self._images[: self._size], device=device),
            actions=torch.as_tensor(self._actions[: self._size], device=device),
            old_log_probs=torch.as_tensor(self._log_probs[: self._size], device=device),
