...
ENV_ID = "HalfCheetah-v5" # add
SEED = 42
NUM_STEPS = 2048
TOTAL_TIMESTEPS = 1000000
GAMMA = 0.99
LEARNING_RATE = 3e-4
HIDDEN_DIM = 256 # 진자는 128
VALUE_COEF = 0.5 # 진자는 0.05
ENTROPY_COEF = 0.0001
MAX_GRAD_NORM = 10
SAVE_INTERVAL_UPDATES = 5 # 진자는 50
EVAL_INTERVAL_UPDATES = 25 # 진자는 100
NUM_EVAL_EPISODES = 10
PT_DIR = Path("pt/halfcheetah_ppo") 
LATEST_CHECKPOINT_PATH = PT_DIR / "ppo_latest.pt" 
BEST_CHECKPOINT_PATH = PT_DIR / "ppo_best.pt" 
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")

#PPO params
PPO_CLIP_COEF = 0.2
PPO_EPOCHS = 10
MINIBATCH_SIZE = 256 # 진자는 64
...
class ActorCritic(nn.Module):
    def __init__(self, observation_dim, action_dim, hidden_dim, action_low, action_high): #low high없앰
            ...
            self.register_buffer("action_scale", torch.as_tensor((action_high - action_low) / 2.0, dtype=torch.float32)) # add
            self.register_buffer("action_bias", torch.as_tensor((action_high + action_low) / 2.0, dtype=torch.float32)) # add
    ...
    def forward(self,states):
          ...
          mean = torch.tanh(raw_mean) * self.action_scale + self.action_bias # add
    ...
    def sample_action(self, states):
          ...
          env_actions = torch.max(torch.min(raw_actions, self.action_bias + self.action_scale), self.action_bias - self.action_scale) # add
...
def collect_rollout(env, model, state, num_steps, device):
          ...
          for _ in range(num_steps):
                ...
                reward_list.append(torch.tensor(reward, dtype=torch.float32, device=device)) # add
...
def train():
    ...
    np.random.seed(SEED)
    PT_DIR.mkdir(parents=True, exist_ok=True) # add
    ...
    try:
        for update_index in range(1, num_updates + 1):
            ...
            if update_index % EVAL_INTERVAL_UPDATES == 0:
                ...
                #     break
                if last_eval_return > best_eval_return: # add
                    best_eval_return = last_eval_return # add
                    save_checkpoint(model, optimizer, update_index, last_eval_return, BEST_CHECKPOINT_PATH) # add


            if update_index % SAVE_INTERVAL_UPDATES == 0:
                save_checkpoint(model, optimizer, update_index, last_eval_return, LATEST_CHECKPOINT_PATH)

        save_checkpoint(model, optimizer, update_index, last_eval_return, LATEST_CHECKPOINT_PATH) # add
